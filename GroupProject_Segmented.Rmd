...
```{r}
model = glm(vSegmentedTF ~  + vGenomeAveLength + vSSoDS + IsZoonotic, family = 'binomial', data = df_hostremoved)
summary(model)

model2 = glm(vSegmentedTF ~ HostOrder + vFamily + vGenomeAveLength + vSSoDS + IsZoonotic, family = 'binomial', data = df_hostremoved)

summary(model2)

model3 = glm(vSegmentedTF ~ vFamily, family = 'binomial', data = df_hostremoved)
summary(model3)
```
